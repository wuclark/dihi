# YouTube ID Server Checker (Edge Extension, MV3, version 1.9.0)
...
On a YouTube Mix/autogenerated playlist URL containing an `RD...` list ID, the popup scans the
loaded Mix panel and offers **Snapshot Mix** or **Snapshot + queue videos**. A snapshot stores
the current ordered video IDs and titles in DIHI with a timestamp; the queue option adds each
video individually because YouTube does not expose the `RD...` playlist to yt-dlp as a normal
playlist.
