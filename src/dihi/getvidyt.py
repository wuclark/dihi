...
class AudioMetadataPostProcessor(PostProcessor):
    ...
    def _clean_filename(cls, src: Path) -> Path:
        """``Title [id].f140.m4a`` → ``Title [id].out.m4a``."""
        return src.with_name(cls._FORMAT_ID_RE.sub('.out', src.name))
...
def build_ydl_opts(
    ...
    *,
    ...
    ydl_opts: Dict[str, Any] = {
        ...
        "outtmpl": "%(channel_id)s/%(id)s/CID_%(channel_id)s.%(upload_date|NA)s.%(title)s [%(id)s].%(ext)s",
    }
